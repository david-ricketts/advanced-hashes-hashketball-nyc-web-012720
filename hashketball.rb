  ...
  game_hash.each do |_place, team|
    team.each do |attribute, data|
      if attribute == :players
        data.each do |player|
          if player[:player_name] == _player_name
...
def shoe_size(_player_name)
  game_hash.each do |_place, team|
    team.each do |attribute, data|
      if attribute == :players
        data.each do |player|
          return player[:shoe] if player[:player_name] == _player_name
        end
      end
    end
  end
...
def player_numbers(team_name)
  numbers = []
  game_hash.each do |_place, team|
    next unless atribute == :player_numbers
    
    team.each do |attribute, data|
      next unless attribute == :players
      
      data.each do |data|
        numebers << data[:numbers]
      end
    end
  end
end
    ...
    game_hash.collect do |place, team|
      team.each do |attribute, _data|
        ...
        game_hash[place][attribute].each do |player|
          next unless player[:player_name] == _player_name
...
def big_shoe_rebounds
  ...
  game_hash.each do |_teamm, game_data|
    game_data[:players].each do |player|
      if player[:shoe] > largest_shoe
        largest_shoe = player[:shoe]
        rebounds = player[:reboudns]
      end
    end
  end
  rebounds
